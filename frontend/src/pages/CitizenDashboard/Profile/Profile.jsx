...
const Profile = ({ setLoading, setLoaderText }) => {
  ...
  const fetchProfile = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8080/citizen/profile", {
        method: "GET",
        headers: {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/citizen/profile/${profile.id}?notificationsEnabled=${notificationsEnabled}`,{
        headers : {
          "Content-Type" : "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      ...
      if (data.success) {
        ...
        setProfile(prev => ({ ...prev, notificationEnabled: notificationsEnabled }));
      } else {
  ...
  return (
    <div className={styles.profile_wrapper}>
      ...
      <div className={`${styles.profile_card} ${styles.hero_card}`}>
        ...
        <div className={styles.hero_right}>
          <div className={styles.hero_header}>
            ...
            <span className={styles.role_badge}>Citizen</span>
          </div>
      ...
      {/* 3. Preferences Card */}
      <div className={styles.profile_card}>
        <div className={styles.preference_header}>
          ...
          <button 
            ...
            disabled={!(profile && notificationsEnabled !== profile.notificationEnabled)}
          >
            Update Preference
          </button>
        </div>
        <p className={styles.card_description} style={{ marginTop: "12px" }}>
          Customize your dashboard preferences and settings.
        </p>
        <div className={styles.preferences_list}>
          <div className={styles.preference_item}>
            <div className={styles.preference_label_group}>
              <span className={styles.preference_name}>Push Notifications</span>
              <span className={styles.preference_desc}>Receive updates about filed complaints</span>
            </div>
