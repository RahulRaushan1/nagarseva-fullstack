...
import styles from "./complaintEdit.module.css";
...
const ComplaintEdit = ({
  ...
  const handleDeleteConfirm = async () => {
    setLoaderText("Deleting complaint...")
  ...
  const handleSubmit = async (e) => {
    ...
    existingImages.forEach((img) => formdata.append("imagePublicIds", img.publicId));
    
    newImages.forEach((img) => formdata.append("newimages", img.file));
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.code === "IMAGE_UPLOAD_LIMIT_EXCEEDED")
    ...
    >
              ...
              <div className={styles.form_group}>
                ...
                <div className={styles.image_previews}>
                  {existingImages.map((img, idx) => (
                    <div
                      key={`existing-${idx}`}
                      className={styles.image_preview_box}
                    >
                      <img
                        src={img.url}
      ...
      {isDeleteModalOpen && (
        <ConfirmationModal
          ...
          message="This action cannot be undone. The complaint and all related attachments will be permanently removed."
