...
const AssignOfficerModal = ({ complaint, onClose, setLoading, setLoaderText, refreshComplaintDetails, refreshComplaintList }) => {
  ...
  const fetchOfficer = async () => {
    ...
    try {
      const response = await fetch(`https://nagarseva-backend-oy56.onrender.com/admin/departments/officers?department=${selectedDept}`,{
        method : "GET",
        headers : {
  ...
  const handleAssign = async (officerId) => {
    ...
    try {
      const response = await fetch(`https://nagarseva-backend-oy56.onrender.com/admin/complaint/${complaint.complaintId}/assign?offId=${officerId}`,{
        method : "PUT",
        headers : {
          "Content-Type" : "Application/json",
          Authorization : `Bearer ${localStorage.getItem("token")}`
